...
func getBaseURL(arg string) string {
	...
	if envBaseURL := os.Getenv("ARTUSERS_BASEURL"); envBaseURL != "" {
		baseurl = envBaseURL
	} else {
		baseurl = arg
	}
...
func getToken(arg string) string {
	...
	if envToken := os.Getenv("ARTUSERS_TOKEN"); envToken != "" {
		token = envToken
	} else {
...
func usage() {
	fmt.Println("ARTUSERS - Artifactory users tool")
	fmt.Println()
	fmt.Println("This tool is used for visualizing Artifactory users.")
	fmt.Println()
	fmt.Println("Usage: artusers <baseurl> <tokenfile>")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("ARTUSERS_BASEURL:  Environment variable that overrides the base URL value.")
	fmt.Println("ARTUSERS_TOKEN:    Environment variable that overrides the token value.")
}
